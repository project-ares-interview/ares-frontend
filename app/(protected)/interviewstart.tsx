...
export default function InterviewStartPage() {
  ...
  const {
    setSettings: setInterviewSettings,
    setAnalysisContext,
    ...settings
  } = useInterviewSettingsStore();
  ...
  const [interviewerMode, setInterviewerMode] = useState(
    settings.interviewer_mode || "team_lead"
  );
  ...
  const onSubmit = async (data: InterviewFormData) => {
    ...
    try {
      ...
      const resume_context =
        analysisResponse?.input_contexts?.raw?.resume_context;

      if (!jd_context || !resume_context) {
        console.error(
          "Failed to get raw context from analysis: jd_context or resume_context missing"
        );
      ...
      const interviewResponse = await interviewService.startInterview(
    ...
    >
      <View style={styles.twoColumnLayout}>
        ...
        <View style={styles.leftColumn}>
          {" "}
          {/* Left column for Interview Settings */}
          <Card containerStyle={styles.card}>
            ...
            {renderSelectButtons(gender, setGender, [
              { label: "남성", value: "male" },
              { label: "여성", value: "female" },
            ])}

            <Text style={styles.label}>지원회사명 📌</Text>
            <Controller
              control={control}
              name="company"
              render={({ field: { onChange, onBlur, value } }) => (
            ...
            <Text style={styles.label}>지원직무 📌</Text>
            <Controller
              control={control}
              name="job_title"
              render={({ field: { onChange, onBlur, value } }) => (
            ...
            <Text style={styles.label}>면접 모드 📌</Text>
            {renderSelectButtons(interviewerMode, setInterviewerMode, [
              { label: "실무 면접", value: "team_lead" },
              { label: "임원 면접", value: "executive" },
            ])}
            ...
            >
              {renderSelectButtons(difficulty, setDifficulty, [
            ...
            <View style={styles.tipBox}>
              <Text style={styles.tipText}>
                난이도 체크✔︎{"\n"}💡보통 난이도: 기본적인 역량 평가{"\n"}
                💡어려움 난이도: 심층적인 역량과 압박 상황에서의 대처 능력 평가
              </Text>
            </View>
        ...
        <View style={styles.rightColumn}>
          {" "}
          {/* Right column for JD and Resume */}
          <Card containerStyle={styles.card}>
            <Card.Title>직무기술서 📌</Card.Title>
            <Card.Divider />
            {renderFileInput("직무기술서(JD)", jdFile, () =>
              pickDocument(setJdFile)
            )}
            <Controller
              name="jd_text"
              control={control}
              render={({ field: { onChange, onBlur, value } }) => (
                <Input
                  label="또는 텍스트로 붙여넣기"
                  placeholder="이곳에 지원회사에서 제공한 직무기술서 내용을 붙여넣거나, 파일이 없는 경우 '없음'이라고 입력하세요."
                  value={value}
                  onChangeText={onChange}
                  onBlur={onBlur}
                  multiline
          ...
          <Card containerStyle={styles.card}>
            <Card.Title>이력서 📌</Card.Title>
            <Card.Divider />
            {renderFileInput("이력서", resumeFile, () =>
            ...
            <Controller
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <Input
                  label="또는 텍스트로 붙여넣기"
                  placeholder="이곳에 본인의 이력서 내용을 붙여넣거나, 파일이 없는 경우 '없음'이라고 입력하세요."
                  value={value}
                  onChangeText={onChange}
                  onBlur={onBlur}
                  multiline
          ...
          <Button
            title={
              isAnalyzing
                ? "작성 내용을 분석중입니다. 잠시만 기다려주세요.\n해당 작업은 작성 내용에 따라 30초~1분 정도 소요됩니다."
                : "면접 시작하기"
            }
...
const styles = StyleSheet.create({
  ...
  selectedButton: {
    backgroundColor: "#4972c3ff", // Darker blue for selected buttons
    borderRadius: 8,
    paddingVertical: 6,
  ...
  fileUploadButton: {
    backgroundColor: "#4972c3ff", // A light blue for file upload buttons
    borderRadius: 8,
    paddingVertical: 6,
    paddingHorizontal: 11,
  },
  ...
  inputField: {
    // New style for regular Input fields
    borderWidth: 1,
    borderColor: "#ddd",
    borderRadius: 8,
  ...
  guideButton: {
    backgroundColor: "#7e91b9ff", // A more prominent blue for the guide button
    paddingVertical: 6,
    paddingHorizontal: 10,
    borderRadius: 5,
  },
